...
3. MMAudio should not be reduced to summary text only.
...
5. QwenVL should be introduced as the primary video-semantic-analysis layer for authoring.
   ...
   - MMAudio-derived summary
   - MMAudio-derived structured timeline
   - MMAudio-derived latent structure cues
...
3. Extract MMAudio video features.
4. Split those outputs into:
   ...
   - authoring package for prompt and lyric drafting:
     ...
     - structured timeline
     - semantic scene cues
5. Run QwenVL video analysis over the same clip to enrich semantic authoring context.
6. Resolve prompt and lyric inputs with either:
   ...
   The authoring contract is built by merging QwenVL scene analysis with MMAudio-derived timing and structure cues.
...
- latent and embedding analysis outputs still influence music generation planning, even if the implementation path reaches ACE-Step through authored text and metadata
